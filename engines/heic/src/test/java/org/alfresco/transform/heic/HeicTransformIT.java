...
@Tag("integration")
@Testcontainers
class HeicTransformIT {

    @Container
    static final GenericContainer<?> engine =
            new GenericContainer<>("angelborroy/alf-tengine-heic:latest")
                    .withExposedPorts(8090)
                    .withEnv("MANAGEMENT_HEALTH_JMS_ENABLED", "false")
                    .waitingFor(Wait.forHttp("/actuator/health").forStatusCode(200));

    @Test
    void transform_heicToJpeg() throws Exception {
        byte[] fileBytes;
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("sample.heic")) {
            assertThat(in).as("sample resource sample.heic not found").isNotNull();
        ...
        String body = "--" + boundary + "\r\n"
                + "Content-Disposition: form-data; name=\"file\"; filename=\"sample.heic\"\r\n"
