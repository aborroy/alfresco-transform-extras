...
@Tag("integration")
@Testcontainers
class MarkdownTransformIT {

    @Container
    static final GenericContainer<?> engine =
            new GenericContainer<>("angelborroy/alf-tengine-markdown:latest")
                    .withExposedPorts(8090)
                    .withEnv("MANAGEMENT_HEALTH_JMS_ENABLED", "false")
                    .waitingFor(Wait.forHttp("/actuator/health").forStatusCode(200));

    @Test
    void transform_markdownToPdf() throws Exception {
        byte[] fileBytes;
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("sample.md")) {
            assertThat(in).as("sample resource sample.md not found").isNotNull();
