...
@Tag("integration")
@Testcontainers
class MsgTransformIT {

    @Container
    static final GenericContainer<?> engine =
            new GenericContainer<>("angelborroy/alf-tengine-msg:latest")
                    .withExposedPorts(8090)
                    .withEnv("MANAGEMENT_HEALTH_JMS_ENABLED", "false")
                    .waitingFor(Wait.forHttp("/actuator/health").forStatusCode(200));

    @Test
    void transform_emlToPdf() throws Exception {
        byte[] fileBytes;
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("sample.eml")) {
            assertThat(in).as("sample resource sample.eml not found").isNotNull();
