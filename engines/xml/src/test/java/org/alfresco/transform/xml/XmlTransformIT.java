...
@Tag("integration")
@Testcontainers
class XmlTransformIT {

    @Container
    static final GenericContainer<?> engine =
            new GenericContainer<>("angelborroy/alf-tengine-xml:latest")
                    .withExposedPorts(8090)
                    .withEnv("MANAGEMENT_HEALTH_JMS_ENABLED", "false")
                    .waitingFor(Wait.forHttp("/actuator/health").forStatusCode(200));

    @Test
    void transform_xmlToMetadata() throws Exception {
        byte[] fileBytes;
        try (InputStream in = getClass().getClassLoader().getResourceAsStream("sample.xml")) {
            assertThat(in).as("sample resource sample.xml not found").isNotNull();
